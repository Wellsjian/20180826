...
> pid = os.fork()
		功能： 创建新的进程
		返回值：整数，如果创建进程失败回一返个负数，如果成功则在原有进程中返回新进程的PID，在新进程中返回0
...
##  multiprocessing 模块创建进程  进程中不能包括输入输出函数
...
功能: 监控IO事件，阻塞等待IO发生
参数：rlist  列表  存放关注的等待发生的IO事件  比如需要别人来协作的,input ,accept 等
      wlist  列表  存放关注的要主动处理的IO事件 比如你自己可以控制的文件 send
