...
>表示层 ：  应用层           Telent  Rlogin SNMP  Gopher
>会话层 ：                  SMTP DNS
>传输层 ：  传输层           TCP   UDP  
>网络层 ：  网际层           IP  ICMP ARP RARP AKP UUCP
>链路层 ：                  FDDI  Ethernet Arpanet 
>物理层 ：  网络接口            
...
1. 视频，群聊，广播传输特点 ： 不保证传输的可靠性，传输过程没有连接和断开，数据收发自由随意。
...
2. Python实现套接字编程：import  socketCK_CLOEXEC',
    'SOCK_DGRAM',
    'SOCK_NONBLOCK',
    'SOCK_RAW',
    'SOCK_RDM
...
```python
sockfd.listen(n)
功能 ： 将套接字设置为监听套接字，确定监听队列大小,一个监听器可以监听多个客户端
参数 ： 监听队列大小
```
4. 等待处理客户端连接请求
...
响应码 ： 
```
1xx  提示信息，表示请求被接收 100继续  101切换协议
2xx  响应成功 200响应成功  201 已创建 202 已接受 203 非授权信息 204无内容 205 重置内容 206部分内容
3xx  响应需要进一步操作，重定向 300多种选择 301永久移动 302临时移动 303查看其他位置 304未修改 305 使用代理 307 临时重定向
4xx  客户端错误
5xx  服务器错误
```
* 响应头：对响应内容的描述
