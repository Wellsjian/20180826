...
>>
>>​					sudo servicel start | stop | restart
>>
>>
...
create database stu default charset utf8 collate utf8_general_ci[cs区分大小写]
```
...
select count(country="蜀国") from sanguo where attack > 200;
```

**2.注意**

```sql
se.g. select name,max(attack) from sanguo;
错误的语法
```
...
```sql
select  分组列,聚合函数(列) from 表名 where group by 分组列,... order by... limit...;
e.g. 求三国中每个国家的总攻击力是多少
select country,sum(attack) from sanguo where group by country;
select country,count(id) as cat from sanguo where gender="m" group by country order by cat desc limit 2;
```

**4.分组查询 - having  筛选**

分组后做组内筛选,一般配合着group by 联用

```sql
e.g. select 分组列 from 表名 where 条件group by 分组列 having 条件 order by ... list 
select country,avg(attack) as avATT from sanguo group by country having avAtt > 105;
```
...
e.g.1.查询时攻击力翻倍 
	...
	3.查询攻击力+100之后大于200的英雄的姓名和国家
	select name,country from sanguo where attack + 100 > 200;
```

### 索引

索引指的是对数据库表的一列或者多列的值进行排序的一种结构.
...
​		1.特点    增加主键之后,主键列会自动增加为索引

​		2.增加主键[索引  ]      已有表添加主键      alter table 表名 add primary key(id)
...
​		   创建标的时候指定唯一性 create tablle xxx( id int primary key auto_incremrnt,  phone varchar(32) unique,)

​		   对已有表指定唯一索引    create unique index 索引名 on 表名(字段名); 

​					create unique index u_name on sanguo(name);
...
+ 创建表同时指定普通索引 create table 表名 (id int ,name varchar, index(name),index(字段名))

	    	* 对已有表进行普通索引添加   create index 索引名 on 表名(字段)
