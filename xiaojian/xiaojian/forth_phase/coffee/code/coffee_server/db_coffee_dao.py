...
class CoffeeDao:
    ...
    def get_last_eqinfo(self):
        sql = '''
         select * from manage_eqstate where id in (select max(id)
          from manage_eqstate group by machine_id);'''
        result = self.db_helper.do_query(sql)
        return result

    #根据机器编号取得设备型号信息
    def get_category_info_by_machine_id(self, m_id):
        sql = '''
        select a.* from manage_eqtype as a, manage_eqinfo as b
